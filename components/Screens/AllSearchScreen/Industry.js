...
// //         <FlatList
// //           data={india.filter((item) => item.industry.toLowerCase().includes(search.toLowerCase()))}
// //           keyExtractor={(item) => item.id}
// //           renderItem={({ item }) => (
// //             <TouchableOpacity onPress={() => handleOnPress(item)} style={styles.box}>
// //               <IndustryList {...item} isSelected={selectedItems.some((selectedItem) => selectedItem.id === item.id)} />
// //             </TouchableOpacity>
// //           )}
// //         />
// //       </View>
...
// // const IndustryList = ({ industry, logo, state, isSelected }) => (
...
const India = ({route}) => {
  ...
  const fetchIndustries = async () => {
    try {
      ...
      const response = await fetch('http://3.27.162.120:8080/filmhook-0.0.1-SNAPSHOT/user/getIndustryByCountry', {
        method: 'POST',
        headers: {
      ...
      if (responseData.status === 1) {
        setIndustries(responseData.data[0].industryList);
      }
    } catch (error) {
...
//   const fetchIndustries = async () => {
//     try {
//       const token = "eyJhbGciOiJIUzUxMiJ9.eyJ1c2VyTmFtZSI6Inlhc3dhbnRoc2hhbmthcjI3MDVAZ21haWwuY29tIiwidXNlclR5cGUiOiJJbmR1c3RyeVVzZXIiLCJpYXQiOjE3MTUyNDg1ODcsImV4cCI6MTcxNTI0OTc4N30.-QhZJuGXUDukK0rfmm6u8O_KApHbOOSwstmsDPMv-D7A7c2vU4a6WcDjNezkpbSrE63E3NZBOYtTAbNYD4mERw"; // Replace this with your actual bearer token
...
//         },
//         body: JSON.stringify({
//           countryIds: [78] // Assuming 78 is the ID for the country
//         }),
//       });
...
//   return (
//     <View>
//       <Text>List of Industries:</Text>
//       <View>
//         {industries.map(industry => (
//           <Text key={industry.id}>{industry.industryName}</Text>
//         ))}
//       </View>
//     </View>
//   );
